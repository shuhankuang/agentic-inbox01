...
- **Draft replies on demand** — Click the AI reply button in an email and the agent reads the thread and drafts a reply. Automatic drafting of every new inbound email is an opt-in per-mailbox setting (off by default), and drafts always require explicit confirmation before sending
